...
| 版本 | 子目标 | 主要改动 |
|---|---|---|
| **v8.1.0** | C | **Magum Trials 分关采集**：`StageProbeServer` 检测 `#LobbyMiniGame == 4` 走 MT 专用路径，tier 维度替换为 `#MagumTrialDifficulty GameScores`，stage holder 复用标准检测；`StagePayload` 加 `inMagumTrials` 字段；`StageBoundaryDispatcher.computeState` 给 MT stageType 加 `mt_` 前缀做命名空间隔离，避免与大厅塔同 ID 子关合桶；`isT1F1` 自动清零排除 `mt_*`；MT 内 `#Floor CTT` ~30 层连续不复位（datapack `break_room_universal` 已显式排除 BR6），逐层独立桶；`floor <= 0` 防御丢弃中央选关区瞬时 0 值；新增 `ServerConfig.collectMagumTrials` 总开关（默认 true）+ `configVersion` 4；HUD 用 `MT D{tier}·F{floor}` 头、聊天栏 header 走专用 i18n key。详见 `docs/MAGUM_TRIALS_STAGE_TRACKING.md` |
...
4. [ ] **MT 跨难度 wipe 钩子**：v8.1.0 暂未实现"切换 MT 难度时清旧难度数据"——见 `docs/MAGUM_TRIALS_STAGE_TRACKING.md` §8.1
5. [ ] **MT 单独历史归档**：v8.1.0 把 MT 跑数据混在主 stage 桶里，未来按需要拆出独立 archive——同上 §8.2
6. [ ] **MT 客户端 fallback**：v8.1.0 仅服务端权威路径识别 MT；客户端 detector 路径（双端 mod 缺一时）暂不识别 MT，全部归 MINIGAME——见同上 §6.4
7. [ ] （你后续添加）
