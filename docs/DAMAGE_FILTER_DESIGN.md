...
> **本文档定位**：v6.7 / v7 阶段引入的"异常大额伤害平滑过滤器"完整设计。在现有 `L9_FILTER`（v6.5.2 固定值黑名单）+ `suspectVictims`（v6.5.8 可疑怪物名单）+ `L8 carry 剥离`（v6.5.9）的基础上，把过滤体系从"硬编码黑名单"演进为"基于上下文的多信号判定 + 自适应 P95 Outlier 检测"。
>
> **关联文档**：
