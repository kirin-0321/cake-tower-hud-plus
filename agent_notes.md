...
- **`ViewStatsRegistry`**：把 datapack 的 ~80 行 `tellraw` 抽成 `List<StatEntry>` 常量表（objective + icon + posColor + negColor）。datapack 改字段 = 改这张表 + 重编。**v8.4.3 起**额外维护 `List<StatusEffectEntry> STATUS_EFFECTS` 表（30+ 个 tag-based 状态：`Berserk` / `Burnt` / `Dizzy` / `Cursed` / `BrokenLeg` ... 等三段：positive/neutral/negative）。
- **`ViewStatsBuilder`**：走表 + 硬编码处理特殊条目（CrackedHearts/PinkHearts、NegMaxHealth、SpeedAmplifier 双向 bold、Size/Gravity 双向 icon、CelestialKarma 受 `#ServerTier CT==3` 门控、`#Skulls CTT` fake player、**v8.4.3 起 `TowersRegen` 走表内 entry + derived 计算 = `Regen + Broccoli + BroccoliW`**）。**v8.4.3 起**在 CelestialKarma 之后、`#Skulls` 之前插入状态效果遍历：用 `player.getCommandTags().contains(tag)` 检测，命中后 `Text.translatable(translateKey).setStyle(...)` 输出 —— 客户端汉化资源包能把 `Berserk` 翻译成 `癫狂` 等本地化文本，与 datapack chat 路径行为完全一致。**跳过**：`Your Stats:` 标题 + `Hover over an icon...` 提示（HUD 不是 chat 没 hover）、Grass Toucher 行、Game Time 行（HUD 无意义）。
...
## v8.4.3 · ViewStatsBuilder 完整还原 datapack 输出

**问题背景**：v8.4.0 实装服务端 push 时**两类条目漏了**，导致 HUD 属性面板比 datapack chat 路径缺行：

1. **`TowersRegen` 死代码**：`ViewStatsRegistry.ENTRIES` 注释里说"builder 自己算"，结果 entry 没列进 ENTRIES → builder 的 `for (entry : ENTRIES)` 循环根本不遍历它 → `if ("TowersRegen".equals(...))` 的 derived 计算分支永远是死代码。表现：玩家有再生 buff 时 HUD 缺一行绿色 `\uE004`（再生图标）。
2. **状态效果整段跳过**：datapack 第 148-181 行的 30+ 个 `tellraw @s[tag=Xxx] {"translate":"Yyy"}` 完全没还原（原 javadoc 明确写"暂不还原"）。表现：玩家身上有"癫狂 / 燃烧 / 眩晕 / 诅咒 / ..." 等状态时 HUD 不显示对应红字 / 警示文字。

**修复**：
- `TowersRegen` 加入 `ENTRIES`（位置：`Healing` 之后、`HealPercent` 之前，对齐 datapack 顺序）；builder 既有 `if ("TowersRegen".equals(...))` 分支无需改动，瞬间复活。
- 新增 `ViewStatsRegistry.StatusEffectEntry` record + `STATUS_EFFECTS` 静态列表（按 datapack 的 positive/neutral/negative 顺序，~30 条）；builder 在 CelestialKarma 之后、`#Skulls` 之前插入 `for (st : STATUS_EFFECTS) { if (player.getCommandTags().contains(st.tag())) lines.add(Text.translatable(st.translateKey()).setStyle(...)) }`。
- **翻译**：用 `Text.translatable(key)` 而不是 `Text.literal(text)` —— `Berserk` 等 translate key 对应 Cake Team Towers 资源包 lang 文件里的本地化条目，客户端汉化包可以把 `Berserk` 翻译成 `癫狂`。datapack chat 路径本来就是 `{"translate":"Berserk"}`，行为完全一致；没装汉化包时 fallback 到英文 key 原文。
- **拼写陷阱**：`D07_Theif` / `Mysterous Vampire Challenge` 在 datapack 中本来就拼错了；翻译 key 必须 1:1 保留，否则匹配不到资源包 lang 文件。

**hoverEvent 仍然不传**：HUD `StatsRenderer` 一行一行画，不是 chat，没有 hover 触发条件，省 ~2 KB/玩家 payload 开销。

**差量缓存兼容**：`PlayerStatsPushBroadcaster.Snapshot` 用 `Text.getString()` 比对字面量，新增的状态行字面量变化也能触发推送，无需修改脏检查逻辑。
